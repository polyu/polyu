...
class cbactions
{
	...
	function create_playlist($params)
	{
		...
		{
			$db->insert(tbl($this->playlist_tbl),array("playlist_name","playlist_describe","userid","date_added","playlist_type"),
									  array($name,$describe,userid(),now(),$this->type));
	...
	function edit_playlist($params)
	{
		...
		{
			$db->update(tbl($this->playlist_tbl),array("playlist_name","playlist_describe"),
									  array($name,$describe)," playlist_id='".$params['pid']."'");
	...
	function get_playlists()
	{
		...
		{
			$result = $db->select(tbl($this->playlist_tbl),"*"," playlist_type='".$this->type."'",false,'date_added desc');
		}
		else
		{
	...
	/**
	 * Function used to get playlitems
	 */
	function get_playlist_items($pid)
	{
		global $db;
		$result = $db->select(tbl($this->playlist_items_tbl),"*","playlist_id='$pid'",false,'priority ASC');
		if($db->num_rows>0)
			return $result;
		else
			return false;
	...
	/**
	 * Function used to get owner name by videoid
	 */
	function get_owner_by_videoid($vid)
	{
		global $db;
		$result_userid = $db->select(tbl('video'),'userid'," videoid='$vid'");
		if($result_userid)
		{
			$userid=$result_userid[0]['userid'];
			$result_owner = $db->select(tbl('users'),'username'," userid='$userid'");
			if($result_owner)
			{
				return $result_owner[0]['username'];
			}
			else return false;
	...
	/**
	 * Function used to get owner name by playlistid
	 */
	function get_owner_by_playlistid($pid)
	{
		global $db;
		$result_userid = $db->select(tbl('playlists'),'userid'," playlist_id='$pid'");
		if($result_userid)
		{
			$userid=$result_userid[0]['userid'];
			$result_owner = $db->select(tbl('users'),'username'," userid='$userid'");
	...
	/**
	 * Function used to get priotiry
	 */
	function get_priority_by_playlist_item_id($vid)
	{
		global $db;
		$result_priority = $db->select(tbl('playlist_items'),'priority'," playlist_item_id='$vid'");
		if($result_priority)
		{
			return $result_priority[0]['priority'];
		}
		else return false;
	}
	
	/**
	 * Function used to set priotiry
	 */
	function set_priority_by_playlist_item_id($vid,$priority)
	{
		global $db;
	   $db->update(tbl('playlist_items'),array('priority'),array($priority)," playlist_item_id='$vid'");
	}
	
	
	/**
	 * Function used to gen playlist
	 */
	function gen_playlist($pid)
	{
	   $items= $this->get_playlist_items($pid);
	   if($items)
	   {
		   $result="[";
		   foreach($items as $item)
		   {
		   		$video=get_videos(array('videoid'=>$item['object_id']));
		   		if($video)
		   		{
		   			if($result=='[')
		   			$result=$result."{";
		   			...
		   			$result=$result.",{";
		   			$result=$result."title:'".$video[0]['title']."',";
