...
class ffmpeg 
{
	 ...
	 */
	 function get_file_info( $path_source =NULL) {
		...
		$this->log('raw_output_for_stat',$output);
		$this->raw_info = $info;
		# parse output
	...
	function parse_format_info( $output ) {
		...
		if( $args = $this->pregMatch( 'video:([0-9]+)kB audio:([0-9]+)kB', $output) ) {
			$video_size = (float)$args[1];
			$audio_size = (float)$args[2];
		} else {
			echo "video pregMatch";
			return false;
		}
