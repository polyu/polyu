...
class CBvideo extends CBCategory
{
	...
	{
		global $eh,$Cbucket,$db,$Upload,$cbvid;		
		...
		if(empty($eh->error_list))
		{
			...
			}elseif(!$this->is_video_owner($vid,userid()) && !has_access('admin_access',TRUE))
			{
				...
				//pr($upload_fields);
				//===hack by jiaqi====
				$pid_temp=mysql_clean($_POST['playlist_id']);
				if($pid_temp!='-1'&&!$cbvid->action->playlist_item_with_obj($vid,$pid_temp))
				{
	...
	{		
		...
		$result = $db->select($tbls,$fields,"playlist_id='$pid' AND ".$vtbl.".videoid=".$ptbl.".object_id",false," priority asc");
		if($db->num_rows>0)
			return $result;
		else
			return false;
