...
if(!empty($mode))
{
	switch($mode)
	{
		...
		{
			if($_POST['itemid'] && $_POST['priority'] && is_numeric ($_POST['itemid']) && is_numeric ($_POST['priority']) )
			{
				$cbvid->action->set_priority_by_playlist_item_id(intval($_POST['itemid']),intval($_POST['priority']));
				echo "ok";
			}
			else echo "fail";
