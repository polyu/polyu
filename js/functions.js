...
function updatePriorityGUI(order,itemid)
{
$.post(page,{mode:"update_priority",itemid:itemid,priority:order},function(data)
{
...
var globalText;
function updatePriority(textObject,itemid)
{
	globalText=$(textObject);
	
	$.post(page,{mode:"update_priority",itemid:itemid,priority:globalText.val()},function(data)
	{
		if(data=="ok")
		{
		globalText.attr("disabled","disabled");
		setTimeout("reopen(globalText)",2000);
		}
		else
		{
		alert("Incorrect Priority");
		globalText.val("0");
		}
	}
	);
}
function reopen(textObject)
{
